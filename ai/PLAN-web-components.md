...
> **rev5 amendment:** The `packages/tokens/` move (step 2) is **cancelled**. Tokens stay at the repository root (`tokens/`, `scripts/`, `build/css/`, `docs/`). Anywhere this document shows a `packages/tokens/...` path, read it as the equivalent repo-root path (e.g. `packages/tokens/build/css/<brand>.css` → `build/css/<brand>.css`). `packages/` contains only `components/` and `mcp/`. There is no `@riverromney/tokens` npm package; consumers keep syncing via `sync-tokens` against repo-root paths.
...
> **Status (rev5): complete.** `list_components`, `get_component`, and `check_usage` are shipped, tested, and serving the committed `design-system.json`. No further investment until external client demand exists — do not add tools, publish to npm, or expand scope speculatively.
...
2. ~~**npm workspaces conversion**~~ — **REMOVED (rev5).** The repository stays single-package with tokens at the root. Rationale: one maintainer/consumer (River); the `sync-tokens` scripts in `portfolio-vercel` and `decisioning-table` already work against current repo-root paths; the move adds a monorepo toolchain plus consumer path-migration churn for no practical solo-practitioner benefit. No `@riverromney/tokens` package is cut; distribution stays "copy `build/css/<brand>.css` via `sync-tokens`". Step numbers below are intentionally left unchanged for traceability with git history, commit messages, and existing cross-references.
...
- ~~**Distribution channel**~~ — **Resolved (rev5):** no npm package, no CDN build. Tokens are consumed by copying `build/css/<brand>.css` via each consumer's `sync-tokens`. Revisit only if a non-River consumer appears.
...
- ~~**`packages/tokens/scripts/codemods/`**~~ — moot under rev5 (step 2 removed; no monorepo move, no codemod scaffolding).
...
### Step polish
- ~~**Distribution channel** is still gating step 2~~ — **Moot (rev5):** step 2 removed; distribution resolved to "`sync-tokens` copies `build/css/<brand>.css`". No npm/CDN until a real external consumer asks.
